from pathlib import Path

from app.extraction.raw_extractor import RawPDFExtractor
from app.extraction.noise_detector import classify_page_noise
from app.extraction.layout_analyzer import analyze_page_layout
from app.services.cleaning_service import clean_document
from app.semantic.processor import process_document
from app.semantic.grouping import group_semantic_blocks


PROJECT_ROOT = Path(__file__).resolve().parent.parent

PDF_FILES = [
    "AMGN-135003565.pdf",
    "UNAM-135051123.pdf",
    "NYLM-134614243.pdf",
]


def count_sections(sections):
    total = len(sections)

    for section in sections:
        total += count_sections(section.children)

    return total


def count_blocks_in_sections(sections):
    total = 0

    for section in sections:
        total += len(section.blocks)
        total += count_blocks_in_sections(section.children)

    return total


def main():
    print("=" * 80)
    print("STAGE 7 END-TO-END PIPELINE TEST")
    print("=" * 80)

    all_passed = True

    for filename in PDF_FILES:
        print()
        print("=" * 80)
        print(f"PROCESSING: {filename}")
        print("=" * 80)

        pdf_path = PROJECT_ROOT / "sample_documents" / filename

        # Stage 4: raw extraction
        extractor = RawPDFExtractor(pdf_path)
        document = extractor.extract()

        # Stage 4: noise and layout analysis
        for page in document.pages:
            classify_page_noise(page)
            analyze_page_layout(page)

        # Stage 5: cleaning
        cleaned_document = clean_document(document)

        # Stage 6: semantic classification
        semantic_blocks = process_document(
            cleaned_document
        )

        # Stage 7: body grouping and hierarchy
        sections = group_semantic_blocks(
            semantic_blocks
        )

        section_count = count_sections(sections)
        grouped_blocks = count_blocks_in_sections(sections)

        raw_pages = document.page_count
        clean_pages = len(cleaned_document.pages)

        print(f"Raw pages:         {raw_pages}")
        print(f"Clean pages:       {clean_pages}")
        print(f"Semantic blocks:   {len(semantic_blocks)}")
        print(f"Sections:          {section_count}")
        print(f"Grouped blocks:    {grouped_blocks}")

        passed = (
            raw_pages == clean_pages
            and len(semantic_blocks) > 0
            and section_count > 0
            and grouped_blocks == len(semantic_blocks)
        )

        if passed:
            print("RESULT: PASS")
        else:
            print("RESULT: FAIL")
            all_passed = False

    print()
    print("=" * 80)

    if all_passed:
        print("STAGE 7 END-TO-END TEST PASSED")
    else:
        print("STAGE 7 END-TO-END TEST FAILED")

    print("=" * 80)


if __name__ == "__main__":
    main()